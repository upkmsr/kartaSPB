...
def test_metro_snapshot_has_lines_stations_and_entrances():
    ...
    counts = {category: sum(item.category_id == category for _, _, item in parsed) for category in {
        "metro-line", "metro-station", "metro-entrance"
    }}
...
def test_metro_import_provenance_and_nearest_analysis():
    ...
    with get_engine().connect() as connection:
        assert connection.execute(text(
            "SELECT count(*) FROM project_objects WHERE source='osm-metro'"
        )).scalar_one() == 339
        assert connection.execute(text(
            "SELECT bool_and(ST_IsValid(geometry)) FROM project_objects WHERE source='osm-metro'"
        )).scalar_one()
