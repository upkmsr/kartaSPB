...
def _stage(connection: Connection, run_id: int, source_record_id: str, raw: dict[str, Any]) -> int:
    ...
    """), {"run": run_id, "record": source_record_id, "raw": json.dumps(raw)}).scalar_one())
        ...
        try:
            for record_id, raw, record in records:
                ...
                normalized = remove_empty({
                    "sourceId": record_id, "name": normalize_text(record.name),
                    "categoryId": record.category_id,
                    "description": normalize_text(record.description),
                    "address": normalize_text(record.address), "aliases": list(record.aliases),
                    "geometry": record.geometry, "properties": record.properties,
                })
                ...
                existing = connection.execute(text(
                    "SELECT 1 FROM project_objects WHERE id=:id"
                ), {"id": object_id}).scalar_one_or_none()
                connection.execute(text("""
                ...
                """), {
                    "id": object_id, "name": record.name, "category": record.category_id,
                    "description": record.description, "geometry": json.dumps(record.geometry),
                    "properties": json.dumps(record.properties), "source": source.id,
                ...
                """), {"object": object_id, "source": source.id, "record": record_id,
                        "run": run_id, "collected": collected, "confidence": record.confidence})
                connection.execute(text("""
                    UPDATE ingestion_staging SET normalized_payload=CAST(:payload AS jsonb),
