...
`app/user_geometries` владеет API и таблицей пользовательских рисунков.
Создание, редактирование и удаление не проходят через ingestion pipeline.
Frontend `domain/userGeometry.ts` хранит правила черновика отдельно от MapLibre;
карта сообщает координаты и показывает отдельный GeoJSON source. Точки,
линии и области включаются через существующий Layer Registry.
