...
После запуска Compose миграция `0013_user_geometries` применяется автоматически.
Откройте интерфейс, раскройте «Мои геометрии» и выберите инструмент. Для линии
нужны два касания, для области — три; кнопка «Сохранить» завершает рисунок.
Для перемещения вершины при редактировании нажмите её маркер, затем новое место.
`Отменить рисование` удаляет только черновик. Пользовательские данные живут в
named PostGIS volume и не затрагиваются командами импорта внешних источников.

Проверка API:

```bash
curl http://localhost:8000/api/user/geometries
docker compose exec backend alembic current
docker compose exec backend alembic heads
```
