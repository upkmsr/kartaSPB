...
## SPRINT 15 — local user data

`user_geometries` — отдельная таблица PostGIS, не связанная с импортируемыми
`project_objects` и `object_provenance`. Локальное приложение пока однопользовательское:
API не имеет учётных записей; при открытии интерфейса на том же сервере
геометрии доступны всем его локальным пользователям. Состояние черновика
принадлежит React, MapLibre передаёт координаты и отображает GeoJSON, а
сервер создаёт UUID и валидирует геометрию в Python и PostGIS.
